...
public class GrpageService implements IDao<Grpage> {
  ...
  public List<Grpage> findByNom(String searchText) {
    List<Grpage> groupes = new ArrayList<>();
    String sql = "SELECT * FROM Grpage WHERE nom_groupe LIKE ?";

    try (PreparedStatement ps = connexion.getCn().prepareStatement(sql)) {
        ps.setString(1, "%" + searchText + "%"); // Recherche partielle
        ResultSet rs = ps.executeQuery();

        while (rs.next()) {
            Grpage groupe = new Grpage(
                rs.getInt("id"),
        ...
        System.out.println("Erreur dans findByNom : " + ex.getMessage());
    }
